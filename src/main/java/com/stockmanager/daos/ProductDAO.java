...
public class ProductDAO {
    ...
    // Add product
    public void addProduct(String name, String categoryId, double price, int stockQuantity) {
        if (!categoryExists(categoryId)) {
            System.out.println("Error: Category with ID " + categoryId
                    + " does not exist. Please create it first or use a valid Category ID.");
            return;
        }

        String sql = "INSERT INTO products (id, name, category_id, price, stock_quantity) VALUES (?, ?, ?, ?, ?)";
        try (Connection conn = DBConnection.getConnection();
                PreparedStatement stmt = conn.prepareStatement(sql)) {
    ...
    public void printAllProducts() {
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            System.out.println(
                    String.format("%-5s | %-20s | %-20s | %-12s | %-10s", "ID", "Name", "Category", "Price", "Stock"));
            System.out.println("-----------------------------------------------------------------------------");
            while (rs.next()) {
                ...
                System.out.println(String.format("%-5s | %-20s | %-20s | $%-11.2f | %-10d",
                        rs.getString("id"), rs.getString("name"), categoryName, rs.getDouble("price"),
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                ...
                while (rs.next()) {
                    ...
                    System.out.println(String.format("%-5s | %-20s | $%-11.2f | %-10d",
                            rs.getString("id"), rs.getString("name"), rs.getDouble("price"),
                            rs.getInt("stock_quantity")));
                }
                if (!found) {
